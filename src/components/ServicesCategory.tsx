...
const ServicesCategory = () => {
  ...
  const PURCHASE_LINKS_BY_LOCATION_AND_NAME: Record<string, Record<string, PurchaseValue>> = {
    // Example mapping by visible location group and plan name. Add your entries here.
    // 'Intel-India': {
    //   'Photon VPS': 1,        // will resolve to cart.php?a=add&pid=1
    //   'Warpdrive VPS': 2,
    // },
  };
  ...
  const resolvePurchaseUrl = (value: PurchaseValue): string => {
    if (typeof value === 'number') {
      return `https://www.matrixclouds.site/billing/cart.php?a=add&pid=${value}`;
    }
    return value;
  };

  const getPurchaseUrl = (service: { id: number; name: string; location: string }): string | undefined => {
    const byId = PURCHASE_LINKS[service.id];
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {filteredServices.map((service) => (
                ...
                >
                  ...
                  {getPurchaseUrl(service) ? (
                    <a
                      href={getPurchaseUrl(service)}
                      target="_blank"
